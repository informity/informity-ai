...
def _get_tool_dispatch() -> tuple[Any, type[Exception]]:
    """Internal helper for get tool dispatch."""
...
async def handle_jsonrpc_request(
    ...
    *,
    ...
    if method == "tools/call":
        ...
        try:
            timeout_seconds = max(
                5.0, float(getattr(settings, "mcp_tool_call_timeout_seconds", 30.0) or 30.0)
            )
            ...
            log.exception(
                "mcp_tool_call_failed", tool_name=tool_name, transport=transport, error=str(exc)
            )
