...
_TAVILY_SEARCH_URL = str(
    os.getenv("INFORMITY_TAVILY_SEARCH_URL") or "https://api.tavily.com/search"
).strip()
_TAVILY_USAGE_URL = str(
    os.getenv("INFORMITY_TAVILY_USAGE_URL") or "https://api.tavily.com/usage"
).strip()
_LINKUP_SEARCH_URL = str(
    os.getenv("INFORMITY_LINKUP_SEARCH_URL") or "https://api.linkup.so/v1/search"
).strip()
...
class TavilyProvider:
    ...
    def _fetch_usage(self, *, timeout_seconds: float) -> tuple[int | None, int | None]:
        """Internal helper for fetch usage."""
        request = urllib.request.Request(
            _TAVILY_USAGE_URL,
            headers={
                "Authorization": f"Bearer {self._api_key}",
        ...
        try:
            with urllib.request.urlopen(
                request, timeout=max(1.0, min(4.0, float(timeout_seconds)))
            ) as response:
                raw = response.read().decode("utf-8")
        ...
        if limit is None:
            limit = _safe_int(account.get("plan_limit")) or _safe_int(account.get("limit"))
        return usage, limit
...
def _extract_usage_from_search_response(
    payload: dict[str, object],
) -> tuple[int | None, int | None]:
    """Internal helper for extract usage from search response."""
    key = payload.get("key")
...
def _classify_tavily_http_error(exc: urllib.error.HTTPError) -> str:
    """Internal helper for classify tavily http error."""
...
def _classify_linkup_http_error(exc: urllib.error.HTTPError) -> str:
    """Internal helper for classify linkup http error."""
    code = int(getattr(exc, "code", 0) or 0)
    if code == 429:
        return "rate_limited"
    if code in {401, 403}:
...
def _extract_linkup_results(payload: object) -> list[SearchResult]:
    """Internal helper for extract linkup results."""
    ...
    for row in candidates:
        if not isinstance(row, dict):
            continue
        title = str(
            row.get("title") or row.get("name") or row.get("source") or row.get("domain") or "",
        ).strip()
...
def _resolve_provider_order() -> list[SearchProviderName]:
    """Internal helper for resolve provider order."""
...
def _build_provider(provider_name: SearchProviderName) -> SearchProvider:
    """Internal helper for build provider."""
...
def _search_with_provider(
    ...
    *,
    ...
    """Internal helper for search with provider."""
