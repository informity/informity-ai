...
9. Do not emit bracketed citation markers or source annotations in the answer body; provenance is shown separately in the UI source cards.
...
_RESEARCHER_AGENT_RAG_PROMPT = """You are a research assistant answering questions from a private document library.
...
Agent mode guidance:
- Treat the retrieval results as an evidence set you are actively exploring.
- When multiple retrieved chunks or subqueries matter, give a deliberate synthesis that integrates them into one answer instead of a terse fragment.
- Prefer a clear structure with short sections or bullets when that makes the answer easier to follow.
- When the user asks for an overview or for all relevant information, be complete before being concise.
- If the evidence is partial or conflicting, say so plainly and separate confirmed facts from uncertainty.
"""
...
MODE_REGISTRY: dict[str, ModeProfile] = {
    ...
    "researcher_agent_rag": ModeProfile(
        id="researcher_agent_rag",
        name="Researcher Agent RAG",
        description="Agent-oriented retrieval-grounded profile for deliberative response generation.",
        identity_prompt=_RESEARCHER_AGENT_RAG_PROMPT,
        mode_policy=_ASSISTANT_MODE_POLICY,
        capabilities=("rag",),
    ),
}
