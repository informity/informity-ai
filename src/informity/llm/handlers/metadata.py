...
"""Module for llm handlers metadata."""
...
_EXTREMUM_TARGET_PATTERN = re.compile(
    r"\b(file|files|document|documents|indexed|modified)\b", re.IGNORECASE
)
_EXTREMUM_RECENT_PATTERN = re.compile(r"\b(most\s+recent|latest|newest)\b", re.IGNORECASE)
_EXTREMUM_OLDEST_PATTERN = re.compile(
    r"\b(oldest|first\s+indexed|earliest\s+indexed)\b", re.IGNORECASE
)
...
def _apply_filename_filter(
    conditions: list[str], params: list[str | int], filename_filter: str
) -> None:
    """Internal helper for apply filename filter."""
...
class MetadataHandler:
    ...
    def matches(self, classification: QueryClassification) -> bool:
        ...
        return classification.intent == QueryType.METADATA or (
    ...
    async def handle(
        ...
        if trace is not None:
            ...
            trace.record(
                "intent",
                {
                    ...
                    # Metadata queries always use metadata query_type.
        ...
        if effective_classification.subtype == QuerySubtype.COMPARATIVE:
            ...
            yield (
                ...
                {"query_type": QueryType.METADATA, "raw_chunks_count": 0},
            )
            yield []
            return
        ...
        if _AGGREGATION_PATTERN.search(question_lower):
            ...
            response = self._format_aggregation_response(
                aggregation, question_lower, as_table=prefer_table
            )
            yield response
            yield (
                StreamSignalTag.METRICS,
        ...
        if _ENUMERATION_PATTERN.search(question_lower):
            ...
            response = self._format_enumeration_response(
                enumeration, question_lower, as_table=prefer_table
            )
            yield response
            yield (
                StreamSignalTag.METRICS,
        ...
        if _COUNT_PATTERN.search(question_lower):
            ...
            response = self._format_count_response(
                count, effective_classification, as_table=prefer_table
            )
            yield response
            yield (
                StreamSignalTag.METRICS,
        ...
        yield (
            "I can help you with file counts, enumerations (years, categories, "
            "file types), aggregations (date ranges, per year), and file listings. "
    ...
    def _extract_explicit_single_year_filter(self, question_lower: str) -> int | None:
        """Internal helper for extract explicit single year filter."""
        if any(
            keyword in question_lower
            for keyword in ("per year", "each year", "all years", "what years")
        ):
            return None
    ...
    async def _get_extremum_file(
        ...
        """Internal helper for get extremum file."""
        conditions: list[str] = []
        params: list[str | int] = []
    ...
    def _format_file_size(self, size_bytes: int) -> str:
        """Internal helper for format file size."""
    ...
    def _format_extremum_missing_response(self, kind: str) -> str:
        """Internal helper for format extremum missing response."""
        if kind == "size":
    ...
    def _format_extremum_response(self, file: IndexedFile, extremum: dict[str, str]) -> str:
        """Internal helper for format extremum response."""
    ...
    async def _get_aggregation(
        ...
        if any(
            keyword in question_lower
            for keyword in ["per year", "from each year", "each year", "grouped by year"]
        ):
    ...
    async def _get_comparative(
        ...
        ascending = any(
            token in question_lower for token in ("fewest", "lowest", "smallest", "least")
        )
        ...
        if "file type" in question_lower or "extension" in question_lower:
            ...
            cursor = await db.execute(
                (
                    ...
                    "WHERE extension IS NOT NULL ORDER BY extension ASC"
                ),
            )
            rows = await cursor.fetchall()
    ...
    def _format_aggregation_response(
        ...
        if as_table:
            ...
            if len(rows) == 2:
                return (
                    "I couldn't determine what aggregation you'd like. Please "
                    "specify: date range or per year counts."
                )
            return "\n".join(rows)
        ...
        if "per_year" in aggregation:
            ...
            if per_year:
                year_items = [
                    f"{item['year']}: {item['count']} file{'s' if item['count'] != 1 else ''}"
                    for item in per_year
                ]
                parts.append(
    ...
    def _format_enumeration_response(
        ...
        if as_table:
            ...
            if "years" in enumeration:
                ...
                rows.append(
                    f"| Years covered | {', '.join(map(str, years)) if years else 'None'} |"
                )
            ...
            if len(rows) == 2:
                return (
                    "I couldn't determine what you'd like to enumerate. "
                    "Please specify: years, categories, or file types."
                )
            return "\n".join(rows)
        ...
        if "years" in enumeration:
            ...
            if is_count_query:
                ...
                if count > 0:
                    ...
                    parts.append(
                        f"You have files from **{count}** year{'s' if count != 1 else ''}: {years_str}"
                    )
                else:
        ...
        if "categories" in enumeration:
            ...
            if is_count_query:
                ...
                if count > 0:
                    ...
                    parts.append(
                        f"You have files in **{count}** categor{'ies' if count != 1 else 'y'}: {categories_str}"
                    )
                else:
        ...
        if "extensions" in enumeration:
            ...
            if is_count_query:
                ...
                if count > 0:
                    ...
                    parts.append(
                        f"You have **{count}** file type{'s' if count != 1 else ''}: {extensions_str}"
                    )
                else:
        ...
        if not parts:
            return (
                "I couldn't determine what you'd like to enumerate. Please "
                "specify: years, categories, or file types."
            )
    ...
    def _format_comparative_response(
        ...
        """Internal helper for format comparative response."""
    ...
    def _format_file_list_response(
        ...
        if total > MAX_FILE_LIST_DISPLAY:
            remaining = total - MAX_FILE_LIST_DISPLAY
            file_items.append(f"\n*... and {remaining} more files*")

        if classification.year_filter:
            header = (
                f"**Found {total} file{'s' if total != 1 else ''} from "
                f"{classification.year_filter}:**\n\n"
            )
        else:
