...
"""Module for llm contract prompt parser."""
...
def extract_required_years(question: str) -> list[int]:
    ...
    return sorted(
        {int(match.group(0)) for match in EXPLICIT_YEAR_PATTERN.finditer(str(question or ""))}
    )


def has_ordered_heading_cue(question: str) -> bool:
    """Has ordered heading cue."""
    ...
    for anchor in anchor_patterns:
        for match in re.finditer(anchor, text, flags=re.IGNORECASE):
            ...
            if "|" in normalized_tail:
                candidates = [
                    part.strip().strip("`").strip() for part in normalized_tail.split("|")
                ]
            else:
                candidates = [
                    part.strip().strip("`").strip() for part in normalized_tail.split(",")
