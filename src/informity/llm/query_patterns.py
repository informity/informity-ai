...
DOCUMENT_TYPES: str = (
    r"(files?|documents?|texts?|records?|entries|items?|sources?|materials?|"
    r"attachments?|notes?|papers?)"
)
...
# Meta query patterns
META_QUERY_PATTERN: str = (
    r"\b(what\s+can\s+you\s+do|how\s+does\s+this\s+work|"
    r"what\s+are\s+your\s+capabilities|help)\b"
)
...
ANALYSIS_ACTION_KEYWORDS: str = (
    r"\b(summarize|summary|overview|breakdown|describe|compare|analyze|"
    r"synthesize|explain|evaluate|assess|review|"
...
AGGREGATE_LISTING_SCOPE_KEYWORDS: str = (
    r"\b("
    r"(which|what)\s+(indexed\s+)?(files?|documents?|records?)\b.*\b("
    r"contain|contains|mention|mentions|include|includes|list)\b"
    r"|"
...
def build_coverage_pattern() -> Pattern[str]:
    ...
    quantifier_pattern = (
        rf"\b{QUANTIFIERS}(\s+(the\s+)?)?(\w+\s+){{0,2}}(years?|{DOCUMENT_TYPES})\b"
    )
