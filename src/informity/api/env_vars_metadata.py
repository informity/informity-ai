...
"""Environment-variable metadata for the application configuration API."""
...
def _format_value(value: object, app_dir: Path | None = None) -> str:
    ...
    if isinstance(value, list):
        parts = [_path_relative_to_app(x, base) if isinstance(x, Path) else str(x) for x in value]
        s = json.dumps(parts)
...
# Section order matches Settings UI: Server → Paths → Privacy → Appearance → Translation → Data
# Sources → Indexing → Embeddings → LLM and RAG → Logging → Diagnostics
_GROUPS: list[tuple[str, str, list[tuple[str, str]]]] = [
    (
        ...
        [
            ...
            (
                "dev_reload",
                "Enable auto-reload on code changes. Development only — leave off in production.",
            ),
            (
                "api_docs_enabled",
                (
                    "Show interactive API docs at /docs. Defaults to enabled only when dev_reload"
                    "is on."
                ),
            ),
        ],
    ...
    (
        "MCP Server",
        (
            "Model Context Protocol server configuration for external AI clients. Use STDIO for"
            "widest client compatibility; HTTP is loopback-only."
        ),
        [
            ...
            (
                "mcp_auto_start",
                (
                    "When true, start MCP server automatically with the application (when MCP is"
                    "enabled)."
                ),
            ),
            ...
            (
                "mcp_scope_mode",
                (
                    "Access level for exposed MCP content: metadata_only, search_snippets, or"
                    "full_content."
                ),
            ),
            (
                "mcp_access_token",
                "Saved MCP HTTP bearer token used when INFORMITY_MCP_TOKEN is not set.",
            ),
            (
                "mcp_tool_call_timeout_seconds",
                (
                    "Maximum MCP tool execution time in seconds before request timeout is"
                    "returned (clamped: 5-120)."
                ),
            ),
            (
                "mcp_http_max_body_bytes",
                "Maximum allowed MCP HTTP request body size in bytes (clamped: 16KB-2MB).",
            ),
        ],
    ),
    (
        "Paths and Storage",
        (
            "Where the application stores database, model files, cache, and logs. Default:"
            "~/.informity. Override via INFORMITY_APP_DATA_DIR."
        ),
        [
            (
                "app_data_dir",
                (
                    "Root directory for all application data — database, models, cache, logs, and"
                    "config."
                ),
            ),
            (
                "cache_dir",
                (
                    "Cache directory for downloaded models and document processing "
                    f"artifacts. Default: app_data_dir/{DirNames.CACHE}."
                ),
            ),
            (
                "db_path",
                f"SQLite database file path. Default: app_data_dir/{DirNames.DB}/{APP_SLUG}.db.",
            ),
            ("logs_dir", "Directory where application log files are written."),
            (
                "models_dir",
                (
                    "Directory for GGUF LLM model files. Default: "
                    f"app_data_dir/{DirNames.MODELS}/{DirNames.LLM}."
                ),
            ),
        ],
    ),
    (
        "Privacy",
        (
            "When full_privacy is true, the app never uses the network; embedding and LLM use"
            "cache or local files only."
        ),
        [
            (
                "full_privacy",
                (
                    "If true, no network access; all operations stay on this computer. Synced to"
                    "embedding_offline and llm_local_only when set via UI."
                ),
            ),
            (
                "embedding_offline",
                (
                    "If true, load embedding model from cache only (no network). Set"
                    "automatically when full_privacy is set."
                ),
            ),
            (
                "llm_local_only",
                (
                    "If true, load LLM only from models_dir (no downloads). Set automatically"
                    "when full_privacy is set."
                ),
            ),
        ],
    ),
    (
        ...
        [
            (
                "ui_theme",
                "Color theme for the app UI. Options: canvas, ember, sage, graphite, onyx.",
            ),
            (
                "enable_menu_bar_icon",
                (
                    "When true, show the menu bar icon while the app is running (macOS desktop"
                    "runtime)."
                ),
            ),
        ],
    ...
    (
        ...
        [
            ...
            (
                ...
                "Default tone for the Translate screen (natural, formal, or literal).",
            ),
            (
                "translate_pinned_languages_limit",
                "Maximum number of additional translation languages shown in the Translate menu.",
            ),
        ],
    ),
    (
        "Data Sources",
        (
            "What to scan and what to skip. List options (ignore_patterns, watched_directories)"
            "are JSON arrays in env vars."
        ),
        [
            ...
            (
                ...
                (
                    "When true, exclude common macOS system and application data (.DS_Store,"
                    "Library/Caches, etc.)."
                ),
            ),
            (
                "exclude_developer_data",
                "When true, exclude common developer data (.git, node_modules, __pycache__, etc.).",
            ),
            (
                "ignore_patterns",
                "Additional glob patterns for files and directories to skip (JSON array in env).",
            ),
    ...
    (
        "Indexing",
        (
            "How document text is split into chunks before embedding, and indexing performance"
            "settings. Affects quality and retrieval."
        ),
        [
            (
                "chunk_child_size_tokens",
                (
                    "Child chunk size in tokens used for precise search matching (typically 1–2"
                    "sentences)."
                ),
            ),
            (
            ...
            (
                "chunk_size_tokens",
                (
                    "Parent chunk size in tokens. Larger values give the LLM more surrounding"
                    "context per retrieved passage."
                ),
            ),
            (
                "enable_ocr_for_images",
                (
                    "When true, enable OCR fallback for image-based documents and image-only PDFs"
                    "when regular text extraction fails."
                ),
            ),
            (
                "entity_extract_acronym",
                "When true, extract acronyms into the term dictionary during indexing.",
            ),
            (
                "entity_extract_location",
                "When true, extract location names into the term dictionary during indexing.",
            ),
            (
                "entity_extract_numeric_id",
                (
                    "When true, extract numeric identifiers (IDs, codes) into the term dictionary"
                    "during indexing."
                ),
            ),
            (
                "entity_extract_organization",
                "When true, extract organization names into the term dictionary during indexing.",
            ),
            (
                "entity_extract_person_name",
                "When true, extract person names into the term dictionary during indexing.",
            ),
            (
                "max_indexable_file_size_mb",
                (
                    "Maximum file size in MB to index. Files larger than this are ignored. Hard"
                    "ceiling: 500 MB."
                ),
            ),
            (
                "scan_file_timeout_seconds",
                "Per-file time limit for extraction in seconds. Hard ceiling: 600 seconds.",
            ),
        ],
    ),
    (
        "Web Search",
        (
            "Third-party web search provider configuration. Used by the assistant when web search"
            "is enabled."
        ),
        [
            ...
            (
                ...
                "Maximum number of web search results to include in the assistant context.",
            ),
    ...
    (
        "Embeddings",
        (
            "Embedding model selection. Task prefixes for indexing and search are applied"
            "automatically."
        ),
        [
    ...
    (
        ...
        [
            (
                "adaptive_rag_tuning",
                (
                    "When true, adapt the number of retrieved chunks based on index size for"
                    "better answer quality."
                ),
            ),
            (
                "chat_auto_continue_enabled",
                "When true, automatically continue long responses that hit the token limit.",
            ),
            (
                "chat_auto_continue_default_max_rounds",
                "Maximum continuation rounds per response when auto-continue is enabled.",
            ),
            (
                "chat_auto_continue_hard_cap",
                "Hard cap on continuation rounds regardless of other settings.",
            ),
            (
                "chat_history_messages",
                (
                    "Number of previous chat messages to include in prompt context. Lower values"
                    "free up tokens for more document context."
                ),
            ),
            (
                "chat_history_messages_assistant",
                (
                    "Assistant-mode history window. Higher values improve conversational"
                    "continuity in assistant mode."
                ),
            ),
            (
                "chat_history_messages_researcher",
                (
                    "Researcher-mode history window. Keep lower to preserve token budget for"
                    "retrieved document context."
                ),
            ),
            (
                "default_chat_mode",
                "Default chat mode shown in the Chat UI: assistant or researcher.",
            ),
            (
                "fts5_candidate_limit",
                (
                    "Maximum extra candidates keyword search (FTS5) can contribute on top of"
                    "vector search results. Set to 0 to disable keyword search augmentation."
                ),
            ),
            ...
            (
                "llm_cpu_threads",
                (
                    "Max CPU threads for LLM generation (0 = auto; set lower to keep system"
                    "responsive during chat)."
                ),
            ),
            (
                "llm_hf_repo",
            ...
            (
                "llm_model_id",
                "Canonical model identifier used for profile matching and diagnostics metadata.",
            ),
            ...
            (
                "ollama_base_url",
                (
                    "Base URL for Ollama API when llm_provider=ollama "
                    f"(default: {config.DEFAULT_OLLAMA_BASE_URL})."
                ),
            ),
            ...
            # NOTE: rag_context_ratio, rag_max_score, rag_top_k, coverage_top_k are model-specific
            # (ModelProfile, not configurable via env)
            (
                "rag_minimal_mode",
                (
                    "When true, use a simplified one-pass retrieval flow instead of the full"
                    "multi-route pipeline. Faster but less adaptive."
                ),
            ),
            (
                "rag_query_rewrite_enabled",
                (
                    "When true, rephrase follow-up questions into self-contained search queries"
                    "for better retrieval in multi-turn conversations."
                ),
            ),
            (
                "rag_rerank",
                "When true, re-rank vector candidates with a cross-encoder before taking top_k.",
            ),
            (
    ...
    (
        ...
        [
            (
                "chat_trace_logging",
                (
                    "When true, write a detailed trace file for each chat message. Useful for"
            ...
            (
                "diagnostics_profile",
                (
                    "Diagnostics profile preset: standard (low overhead), troubleshooting (richer"
                    "diagnostics), custom (manual override)."
                ),
            ),
            (
                "enable_raw_output_control",
                (
                    "When true, show a toggle to view the raw unprocessed model output for each"
                    "response. Useful for debugging reasoning models."
                ),
            ),
    ...
    (
        ...
        [
            (
                "diagnostics_dir",
                "Directory for diagnostics data (quality evaluation runs, traces, reports).",
            ),
        ],
    ),
]
...
_RUNTIME_ENV_VARS: list[tuple[str, str]] = [
    ...
    (
        "INFORMITY_MCP_TOKEN",
        (
            "Optional MCP HTTP bearer-token override. If set, this value is used instead of the"
            "saved MCP access token."
        ),
    ),
]
...
def _build_env_var_items(
    ...
    for field, desc in sorted(variables, key=lambda x: _env_name(x[0])):
        ...
        try:
            ...
            current_value = "(unset)"
        items.append(
            EnvVarItem(name=_env_name(field), current_value=current_value, description=desc)
        )
    return items
...
def get_env_vars_response(settings: object) -> EnvVarsResponse:
    ...
    for field in missing_fields:
        try:
            ...
            if _should_redact_setting_field(field):
                current_display = (
                    _SENSITIVE_ENV_VALUE_MARKER if str(current_display).strip() else ""
                )
        except (AttributeError, TypeError):
        ...
        groups.append(
            EnvVarGroup(
                ...
                description=(
                    ...
                    "adaptive top-k, and relevance thresholds. Leave at defaults unless "
                    "tuning RAG behavior."
                ),
    ...
    if internal_items or leftover_items:
        groups.append(
            EnvVarGroup(
                ...
                description=(
                    "Internal runtime defaults for numeric extraction guards, fit-to-budget "
                    "rollout, "
    ...
    groups.append(
        EnvVarGroup(
            ...
            description=(
                "Runtime-only environment variables used by launch wrappers and"
