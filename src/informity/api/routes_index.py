...
async def rebuild_index(
    ...
    async with REBUILD_GUARD.slot():
        ...
        async with op_state.get_scan_operation_lock():
            ...
            # Check if a scan/rebuild is already running and resolve it
            # (cancel / mark stale / block).
...
def _count_files_under(path: Path) -> int:
    """Internal helper for count files under."""
    if not path.exists():
        return 0
    total = 0
...
async def get_index_status(
    ...
    # During reset, avoid DB reads from polling clients
    # (Settings page polls every 500ms) so reset compaction can obtain
    # the exclusive lock for VACUUM.
    ...
    async with op_state.get_scan_operation_lock():
        ...
        if (source_provider is None) != (entity_type is None):
            ...
            raise HTTPException(
                status_code=400,
                detail=(
                    "source_provider and entity_type must be provided together for scoped reset."
                ),
            )
...
async def _run_reset_task(
    *,
    ...
    """Internal helper for run reset task."""
    clear_contextvars()
    ...
    try:
        ...
        if not scoped_reset:
            ...
            if uploads_dir.exists():
                try:
                    upload_storage_files_deleted = await asyncio.to_thread(
                        _count_files_under, uploads_dir
                    )
            ...
            if not bool(db_counts.get("storage_compacted", False)):
                db_counts["storage_compacted"] = bool(
                    post_compaction.get("storage_compacted", False)
                )
...
async def _run_rebuild_task(scan_id: int) -> None:
    ...
    """Internal helper for run rebuild task."""
    ...
    try:
        ...
        async with op_state.get_ingestion_lock():
            ...
            for indexed_file in all_files:
                ...
                try:
                    ...
                    # Catch exceptions from reindex_file to prevent one file
                    # from stopping the rebuild.
                    errors += 1
                    errors_by_extension[scanned.extension] += 1
                    log.error(
        ...
        await emit_log_event(
            ...
            source="Indexer",
            message=(
                f"Index refresh completed. {files_scanned} files checked, {files_indexed} indexed."
            ),
            scan_id=scan_id,
