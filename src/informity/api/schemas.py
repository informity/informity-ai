...
"""Pydantic API schemas shared between the frontend and backend."""
...
class ChatRequest(BaseModel):
    ...
    scoped_file_ids: list[int] | None = Field(
        default=None, min_length=1
    )  # Optional one-or-more file scope for researcher retrieval
    scoped_upload_ids: list[str] | None = Field(
        default=None, min_length=1
    )  # Optional one-or-more upload IDs for chat-scoped attachments
...
class FileTypeOption(BaseModel):
    """
    One file type option (id, label, extensions).

    Canonical source: file_types.get_file_type_options().
    """
...
class SettingsResponse(BaseModel):
    ...
    enable_ocr_for_images: bool = (
        True  # Enable OCR fallback for image-based documents and image-only PDFs by default
    )
    ...
    enable_raw_output_control: bool = (
        False  # Show control to fetch raw model output per assistant message
    )
    ...
    enable_menu_bar_icon: bool = (
        False  # Show menu bar icon while app is running (macOS desktop runtime)
    )
    cpu_priority_nice: int = 10  # 0 = off, >0 lowers process priority at startup
    translate_default_language: str = "Spanish"  # Primary target language for translation screen
    translate_default_tone: str = "natural"  # Default tone for translation screen
    translate_pinned_languages: list[str] = Field(
        default_factory=list
    )  # Additional translation languages for quick picks
...
class SettingsUpdateRequest(BaseModel):
    ...
    chunk_filter_header_ratio: float | None = (
        None  # Threshold: chunks with >N% header/separator lines are considered header-only
    )
    chunk_filter_min_content_chars: int | None = (
        None  # Minimum content length (chars) to avoid filtering
    )
    ...
    enable_ocr_for_images: bool | None = (
        None
        # Enable OCR for image-based documents and image-only PDFs when regular extraction fails
    )
    ...
    enable_raw_output_control: bool | None = (
        None  # Show control to fetch raw model output per assistant message
    )
    ui_theme: str | None = None  # Color theme: canvas, ember, sage, graphite, onyx
    enable_menu_bar_icon: bool | None = (
        None  # Show menu bar icon while app is running (macOS desktop runtime)
    )
