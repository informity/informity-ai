...
"""Settings API routes for reading and updating application configuration."""
...
def _normalize_enabled_specialization_ids(values: object, *, strict: bool = True) -> list[str]:
    ...
    if not isinstance(values, list):
        if strict:
            raise HTTPException(
                status_code=400,
                detail="enabled_specialization_ids must be a list of specialization IDs",
            )
        return []
    allowed = _visible_specialization_ids()
    seen: set[str] = set()
    normalized: list[str] = []
    for raw in values:
        specialization_id = str(raw or "").strip()
        if not specialization_id:
            continue
        if specialization_id in seen:
            continue
        if specialization_id not in allowed:
            if strict:
                raise HTTPException(
                    status_code=400,
                    detail=(
                        f"Unknown specialization ID in enabled_specialization_ids:"
                        f"{specialization_id}"
                    ),
                )
            continue
...
_SETTINGS_RANGE_RULES: dict[str, tuple[float, float, str]] = {
    ...
    "chunk_filter_header_ratio": (
        0.0,
        1.0,
        "chunk_filter_header_ratio must be between 0.0 and 1.0",
    ),
    ...
    "chat_history_messages_assistant": (
        0,
        20,
        "chat_history_messages_assistant must be between 0 and 20",
    ),
    "chat_history_messages_researcher": (
        0,
        10,
        "chat_history_messages_researcher must be between 0 and 10",
    ),
...
_UPDATABLE_FIELDS: set[str] = {
    ...
    "ollama_timeout_seconds",
    "llm_model_filename",
    # NOTE: rag_max_score and rag_context_ratio are now model-specific (in ModelProfile, not
    # updatable)
    "rag_minimal_mode",
    "rag_minimal_answerability_threshold_focused",
    "rag_minimal_answerability_threshold_coverage",
...
async def get_settings() -> SettingsResponse:
    ...
    return SettingsResponse(
        ...
        web_search_configured=(
            bool(str(s.tavily_api_key or "").strip()) or bool(str(s.linkup_api_key or "").strip())
        ),
        web_search_primary_provider=(
            "linkup"
        ...
        ollama_base_url=str(
            getattr(s, "ollama_base_url", config.DEFAULT_OLLAMA_BASE_URL)
            or config.DEFAULT_OLLAMA_BASE_URL
        ).strip(),
        ...
        translate_pinned_languages=_normalize_additional_translate_languages(
            ...
            limit=_translate_pinned_language_limit(
                getattr(s, "translate_pinned_languages_limit", 6)
            ),
        ),
        translate_pinned_languages_limit=_translate_pinned_language_limit(
            getattr(s, "translate_pinned_languages_limit", 6)
        ),
    )
...
async def update_settings(request: SettingsUpdateRequest) -> SettingsResponse:
    ...
    async with _CONFIG_FILE_ASYNC_LOCK:
        ...
        for field_name, value in updates.items():
            ...
            # NOTE: rag_max_score and rag_context_ratio validation removed — they're now
            # model-specific (not updatable)
            if field_name == "rag_reranker_model" and value is not None:
                value = (value or "").strip()
                if not value:
                    raise HTTPException(
                        status_code=400, detail="rag_reranker_model cannot be empty"
                    )
            if field_name == "ollama_base_url" and value is not None:
                ...
                if not (value.startswith("http://") or value.startswith("https://")):
                    raise HTTPException(
                        status_code=400,
                        detail="ollama_base_url must start with http:// or https://",
                    )
            ...
            if field_name == "pdf_extraction_strategy_order" and value is not None:
                if not isinstance(value, list):
                    raise HTTPException(
                        status_code=400, detail="pdf_extraction_strategy_order must be a list"
                    )
                ...
                if not normalized:
                    raise HTTPException(
                        status_code=400, detail="pdf_extraction_strategy_order cannot be empty"
                    )
            ...
            if field_name == "mcp_access_token" and value is not None:
                value = str(value).strip()
            if field_name == "enable_specializations":
                config.settings.enable_specializations = bool(value)
                config_data["enable_specializations"] = bool(value)
                continue
            if field_name == "enabled_specialization_ids":
                normalized_specialization_ids = _normalize_enabled_specialization_ids(value)
                setattr(config.settings, field_name, normalized_specialization_ids)
            ...
            if field_name == "llm_model_filename" and value is not None:
                ...
                if not value:
                    raise HTTPException(
                        status_code=400, detail="llm_model_filename cannot be empty"
                    )
                if not value.endswith(".gguf"):
                    raise HTTPException(
                        status_code=400, detail="llm_model_filename must be a .gguf file"
                    )
                available_models = await asyncio.to_thread(_list_available_models)
                ...
                for p in value:
                    ...
                    if not exists:
                        raise HTTPException(
                            status_code=400,
                            detail=(
                                f"Watched directory does not exist: {path}. Use an absolute"
                                f"path (e.g. /Users/you/Documents)."
                            ),
                        )
                    if not path.is_dir():
                        raise HTTPException(
        ...
        if diagnostics_profile_value is not None:
            ...
            if preset_values:
                for preset_field, preset_value in preset_values.items():
                    diagnostics_preset_originals[preset_field] = getattr(
                        config.settings, preset_field
                    )
        ...
        mcp_transport = (
            str(getattr(config.settings, "mcp_transport", "stdio") or "stdio").strip().lower()
        )
        ...
        if config.settings.chunk_overlap_tokens >= config.settings.chunk_size_tokens:
            raise HTTPException(
                status_code=400,
                detail=(
                    f"chunk_overlap_tokens ({config.settings.chunk_overlap_tokens}) must be"
                    f"less than chunk_size_tokens ({config.settings.chunk_size_tokens})"
                ),
            )
    ...
    if (
        "watched_directories" in updates
        or "supported_extensions" in updates
        or "exclude_macos_system" in updates
        or "exclude_developer_data" in updates
        or "ignore_patterns" in updates
    ):
