...
_UNFENCED_CODE_LINE_HINT_RE = re.compile(
    r"^\s*(?:"
    r"(?:const|let|var|function|class|interface|type|enum|import|export|from|if|"
    r"else|for|while|switch|case|return|try|catch|finally|throw|new)\b"
...
def _split_table_cells(line: str) -> list[str]:
    """Internal helper for split table cells."""
    text = line.strip()
    if text.startswith("|"):
        text = text[1:]
    if text.endswith("|"):
...
def _flush_text(lines: list[str], blocks: list[dict[str, object]]) -> None:
    """Internal helper for flush text."""
    if not lines:
        return
    markdown = "\n".join(lines).strip("\n")
    if markdown:
...
def _trim_trailing_divider(lines: list[str]) -> None:
    """Internal helper for trim trailing divider."""
    while lines:
...
def _infer_unfenced_code_language(lines: list[str]) -> str | None:
    """Internal helper for infer unfenced code language."""
...
def _looks_like_unfenced_code_block(answer: str) -> bool:
    """Internal helper for looks like unfenced code block."""
    ...
    for line in non_empty:
        ...
        if _UNFENCED_CODE_INLINE_HINT_RE.search(stripped):
            symbol_lines += 1
        if stripped.endswith((".", "?", "!")) and not _UNFENCED_CODE_INLINE_HINT_RE.search(
            stripped
        ):
            prose_like_lines += 1
...
def build_display_blocks(cleaned_answer: str) -> list[dict[str, object]]:
    ...
    while i < len(lines):
        ...
        if has_next and "|" in line and TABLE_DIVIDER_PATTERN.match(lines[i + 1].strip()):
        ...
        if disclaimer_match:
            ...
            while i < len(lines):
                ...
                if (
                    ...
                    or _QUOTE_LINE_RE.match(lines[i])
                    or _CODE_FENCE_OPEN_RE.match(lines[i].strip())
                ):
                    break
