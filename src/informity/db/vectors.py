...
"""SQLite-backed vector storage utilities for indexed document embeddings."""
...
class VectorStore:
    ...
    def _get_thread_connection(self) -> sqlite3.Connection:
        ...
        if not _load_sqlite_vec_extension_sync(conn):
            log.error(
                "sqlite_vec_extension_not_loaded", action="vector_store_connection_init_failed"
            )
    ...
    def fts5_augment_candidates(
        ...
        file_path_select = (
            "file_path AS file_path" if "file_path" in fts_columns else "'' AS file_path"
        )
        filename_select = "filename AS filename" if "filename" in fts_columns else "'' AS filename"
        select_fields = f"chunk_id, file_id, {file_path_select}, {filename_select}, chunk_text"
