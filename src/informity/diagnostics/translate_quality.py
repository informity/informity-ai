"""Module for diagnostics translate quality."""
...
def _normalize_tokens(text: str) -> set[str]:
    """Internal helper for normalize tokens."""
...
def _adjacent_overlap_score(texts: list[str]) -> float | None:
    """Internal helper for adjacent overlap score."""
    if len(texts) < 2:
        return None
    scores = []
...
def _markdown_issue_count(text: str) -> int:
    """Internal helper for markdown issue count."""
    issues = 0
    if not text.strip():
        issues += 1
    if text.count("```") % 2 != 0:
