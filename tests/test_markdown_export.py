"""Test module for tests test markdown export."""
...
def test_render_full_chat_markdown_renders_frontmatter_and_deduplicated_sources() -> None:
    """Test render full chat markdown renders frontmatter and deduplicated sources."""
    ...
    output = render_full_chat_markdown(
        ...
        options=MarkdownExportOptions(
            include_frontmatter=True, template="full_transcript", generated_at=fixed_now
        ),
    )
...
def test_render_current_answer_markdown_is_deterministic() -> None:
    """Test render current answer markdown is deterministic."""
    answer = ChatMessage(
        chat_id="chat-1",
        role="assistant",
    ...
    output = render_current_answer_markdown(
        ...
        chat_mode="researcher",
        options=MarkdownExportOptions(
            include_frontmatter=True, template="concise_summary", generated_at=fixed_now
        ),
    )
    assert output.startswith("---\n")
    assert "generated_at: 2026-05-20T12:00:00+00:00" in output
...
def test_render_full_chat_markdown_supports_concise_summary_template() -> None:
    """Test render full chat markdown supports concise summary template."""
    fixed_now = datetime(2026, 5, 20, 12, 0, 0, tzinfo=UTC)
    output = render_full_chat_markdown(
        chat_title="My Chat",
        chat_id="chat-2",
        ...
        chat_mode="assistant",
        options=MarkdownExportOptions(
            include_frontmatter=False, template="concise_summary", generated_at=fixed_now
        ),
    )
...
def test_render_full_chat_markdown_separates_user_and_assistant_blocks() -> None:
    """Test render full chat markdown separates user and assistant blocks."""
    fixed_now = datetime(2026, 5, 20, 12, 0, 0, tzinfo=UTC)
    output = render_full_chat_markdown(
        chat_title="Chat",
        chat_id="chat-3",
        ...
        chat_mode="researcher",
        options=MarkdownExportOptions(
            include_frontmatter=False, template="full_transcript", generated_at=fixed_now
        ),
    )
...
def test_build_markdown_filename_uses_slug_and_timestamp() -> None:
    """Test build markdown filename uses slug and timestamp."""
