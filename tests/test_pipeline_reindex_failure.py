"""Test module for tests test pipeline reindex failure."""
...
async def test_chunk_embed_store_fails_when_no_embeddable_child_chunks_exist(
    ...
    """Test chunk embed store fails when no embeddable child chunks exist."""
    ...
    monkeypatch.setattr(
        "informity.indexer.pipeline.chunk_text", lambda *_args, **_kwargs: [parent_chunk]
    )
    monkeypatch.setattr(
        "informity.indexer.pipeline.create_child_chunks", lambda _parents: [child_chunk]
    )
    monkeypatch.setattr(
        "informity.indexer.pipeline.insert_chunks_batch", AsyncMock(return_value=[11])
    )
    ...
    monkeypatch.setattr(
        "informity.indexer.pipeline.vector_store.store_embeddings_async", store_embeddings_mock
    )
...
@pytest.mark.asyncio
async def test_reindex_file_preserves_file_row_on_chunk_store_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test reindex file preserves file row on chunk store failure."""
    scanned = ScannedFile(
    ...
    monkeypatch.setattr(
        "informity.indexer.pipeline.get_file_by_source_identity", AsyncMock(return_value=existing)
    )
    ...
    monkeypatch.setattr(
        "informity.indexer.pipeline.classify_file",
        lambda _path, _ext: SimpleNamespace(value="plaintext"),
    )
    ...
    monkeypatch.setattr(
        "informity.indexer.pipeline._build_file_metadata",
    ...
    monkeypatch.setattr(
        "informity.indexer.pipeline.vector_store.delete_by_file_id", lambda _file_id: None
    )
    monkeypatch.setattr(
