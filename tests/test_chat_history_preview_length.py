"""Test module for tests test chat history preview length."""
...
async def test_get_chats_keeps_full_preview_text_without_backend_truncation(
    ...
    """Test get chats keeps full preview text without backend truncation."""
    ...
    try:
        ...
        await db.execute(
            "UPDATE chat_messages SET created_at = ? WHERE id = ?", (base.isoformat(), int(user.id))
        )
        await db.execute(
