...
async def test_translate_chat_message_persists_translation_metadata(
    ...
    try:
        ...
        assert [message.role for message in history] == [ChatRole.USER, ChatRole.ASSISTANT, ChatRole.ASSISTANT]
...
async def test_translate_chat_message_reuses_existing_translation_and_rejects_translated_reply(
    ...
    try:
        ...
        history = await get_chat(db, chat_id)
        translations = [message for message in history if message.translated_from_message_id == int(source_message.id or 0)]
