...
"""Test module for tests integration test retrieval."""
...
async def test_year_filter_2021_returns_only_2021_documents(db):
    ...
    results = await retrieve_chunks(
        query="revenue operating expenses",
        top_k=10,
        year_filter=2021,
        db=db,
    )
...
async def test_year_filter_2022_returns_only_2022_documents(db):
    ...
    """
    results = await retrieve_chunks(
        query="revenue net income quarterly",
        top_k=10,
        year_filter=2022,
        db=db,
    )
...
async def test_year_filter_2023_returns_only_2023_documents(db):
    ...
    results = await retrieve_chunks(
        query="planning meeting quarterly notes",
        top_k=10,
        year_filter=2023,
        db=db,
    )
...
async def test_year_2021_and_2022_result_sets_are_disjoint(db):
    ...
    assert not overlap, f"Chunk IDs appear in both year=2021 and year=2022 results: {overlap}"
...
async def test_filename_filter_employee_handbook(db):
    ...
    results = await retrieve_chunks(
        query="employee guidelines",
        top_k=10,
        filename_filter="employee_handbook.pdf",
        db=db,
    )
    assert len(results) >= 1
    for chunk in results:
...
async def test_filename_filter_annual_report_2022(db):
    ...
    results = await retrieve_chunks(
        ...
        top_k=10,
        filename_filter="annual_report_2022.pdf",
        db=db,
    )
    assert len(results) >= 1
    for chunk in results:
...
async def test_filename_filter_no_match_returns_empty(db):
    ...
    results = await retrieve_chunks(
        query="overview",
        top_k=10,
        filename_filter="nonexistent_file_xyz.pdf",
        db=db,
    )
    assert results == []
...
async def test_extension_filter_pdf_returns_only_pdf_files(db):
    ...
    assert len(results) >= 1
    for chunk in results:
        assert chunk["filename"].endswith(".pdf"), f"Expected .pdf file, got {chunk['filename']!r}"
...
async def test_extension_filter_txt_returns_only_text_files(db):
    ...
    assert len(results) >= 1
    for chunk in results:
        assert chunk["filename"].endswith(".txt"), f"Expected .txt file, got {chunk['filename']!r}"
...
async def test_coverage_query_returns_multiple_distinct_files(db):
    ...
    """
    results = await retrieve_chunks(
        query="company documents overview",
        top_k=15,
        query_type="coverage",
        db=db,
    )
...
async def test_coverage_year_filter_restricts_files(db):
    ...
    assert len(results) >= 1
    for chunk in results:
        assert "2022" in chunk["filename"], f"Coverage leaked non-2022 file: {chunk['filename']!r}"
