"""Test module for tests test routes scan file reindex ops."""
...
async def test_file_reindex_task_marks_failed_on_unhandled_exception(
    ...
    """Test file reindex task marks failed on unhandled exception."""
    ...
    async def _fake_get_connection() -> _DummyDb:
        """Internal helper for fake get connection."""
        return _DummyDb()
    ...
    monkeypatch.setattr(
        routes_scan, "scanned_file_for_path", lambda _path: SimpleNamespace(size_bytes=1)
    )
