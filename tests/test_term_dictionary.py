"""Test module for tests test term dictionary."""
...
@pytest.mark.asyncio
async def test_expand_query_for_retrieval_applies_tiered_expansion(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test expand query for retrieval applies tiered expansion."""
    monkeypatch.setattr(settings, "term_dictionary_enabled", True)

    async def _fake_version(_db: aiosqlite.Connection) -> int:
        """Internal helper for fake version."""
        return 3

    async def _fake_rows(_db: aiosqlite.Connection) -> list[dict]:
        """Internal helper for fake rows."""
        return [
            {
                "alias": "roi",
...
async def test_rebuild_term_dictionary_extracts_acronym_definition_pairs(
    ...
    """Test rebuild term dictionary extracts acronym definition pairs."""
    monkeypatch.setattr(settings, "term_dictionary_enabled", True)
...
def test_expand_query_for_routing_applies_high_confidence_and_cap(
    ...
    """Test expand query for routing applies high confidence and cap."""
    monkeypatch.setattr(settings, "term_dictionary_enabled", True)
...
@pytest.mark.asyncio
async def test_expand_query_for_retrieval_fuzzy_cap_sets_flag(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test expand query for retrieval fuzzy cap sets flag."""
    monkeypatch.setattr(settings, "term_dictionary_enabled", True)
...
@pytest.mark.asyncio
async def test_expand_query_for_retrieval_skips_generic_alias_noise(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test expand query for retrieval skips generic alias noise."""
    monkeypatch.setattr(settings, "term_dictionary_enabled", True)
...
async def test_expand_query_for_retrieval_disabled_is_noop(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test expand query for retrieval disabled is noop."""
...
def test_term_dictionary_builder_excludes_person_name_pairs_by_default(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test term dictionary builder excludes person name pairs by default."""
...
def test_term_dictionary_builder_ocr_normalization_extracts_split_acronym() -> None:
    """Test term dictionary builder ocr normalization extracts split acronym."""
    out: dict[tuple[str, str], term_builder._Candidate] = {}
    term_builder._extract_candidates_from_chunk(
...
def test_term_dictionary_builder_person_name_extractor_is_optional(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test term dictionary builder person name extractor is optional."""
...
def test_term_dictionary_builder_person_name_with_field_label_cue_is_accepted(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test term dictionary builder person name with field label cue is accepted."""
    monkeypatch.setattr(settings, "entity_extract_acronym", False)
    monkeypatch.setattr(settings, "entity_extract_person_name", True)
    out: dict[tuple[str, str], term_builder._Candidate] = {}
    term_builder._extract_candidates_from_chunk(
        content="Employee Name Maria Garcia Job Title Analyst",
        file_id=1,
        chunk_id=1,
        out=out,
    )
...
def test_term_dictionary_builder_person_name_middle_initial_maps_to_same_canonical(
    ...
    """Test term dictionary builder person name middle initial maps to same canonical."""
    monkeypatch.setattr(settings, "entity_extract_acronym", False)
    monkeypatch.setattr(settings, "entity_extract_person_name", True)
    out: dict[tuple[str, str], term_builder._Candidate] = {}
    term_builder._extract_candidates_from_chunk(
...
def test_term_dictionary_builder_person_name_rejects_document_phrase(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test term dictionary builder person name rejects document phrase."""
    monkeypatch.setattr(settings, "entity_extract_acronym", False)
    monkeypatch.setattr(settings, "entity_extract_person_name", True)
    out: dict[tuple[str, str], term_builder._Candidate] = {}
    term_builder._extract_candidates_from_chunk(
        content="Additional Medicare Tax was updated in this report section.",
        file_id=1,
        chunk_id=1,
        out=out,
    )
    assert ("person_name", "additional medicare") not in out


def test_term_dictionary_builder_person_name_rejects_non_person_bill_phrase(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test term dictionary builder person name rejects non person bill phrase."""
    monkeypatch.setattr(settings, "entity_extract_acronym", False)
    monkeypatch.setattr(settings, "entity_extract_person_name", True)
    out: dict[tuple[str, str], term_builder._Candidate] = {}
    term_builder._extract_candidates_from_chunk(
        content="Assembly Bill was reviewed and approved in the hearing.",
        file_id=1,
        chunk_id=1,
        out=out,
    )
    assert ("person_name", "assembly bill") not in out


def test_term_dictionary_builder_person_name_repeated_mention_without_name_cue_is_rejected(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test term dictionary builder person name repeated mention without name cue is rejected."""
    monkeypatch.setattr(settings, "entity_extract_acronym", False)
    monkeypatch.setattr(settings, "entity_extract_person_name", True)
    out: dict[tuple[str, str], term_builder._Candidate] = {}
    term_builder._extract_candidates_from_chunk(
        content="Maria Garcia followed up with finance. Later, Maria Garcia signed the response.",
        file_id=1,
        chunk_id=1,
        out=out,
    )
...
def test_term_dictionary_builder_person_name_detects_camel_joined_label(
    ...
    """Test term dictionary builder person name detects camel joined label."""
    monkeypatch.setattr(settings, "entity_extract_acronym", False)
    monkeypatch.setattr(settings, "entity_extract_person_name", True)
    out: dict[tuple[str, str], term_builder._Candidate] = {}
    term_builder._extract_candidates_from_chunk(
...
def test_term_dictionary_quality_gate_fails_for_high_noise_rate() -> None:
    """Test term dictionary quality gate fails for high noise rate."""
