"""Test module for tests test chat upload delete lifecycle."""
...
async def test_delete_chat_upload_prunes_empty_chat_upload_dir(
    ...
    """Test delete chat upload prunes empty chat upload dir."""
    ...
    try:
        ...
        attachment = await get_chat_upload_attachment_by_upload_id(
            db, upload_id=upload_id, chat_id=chat_id
        )
...
async def test_delete_chat_upload_marks_failed_when_index_record_delete_fails(
    ...
    """Test delete chat upload marks failed when index record delete fails."""
    app_data_dir = tmp_path / "app-data"
    app_data_dir.mkdir(parents=True, exist_ok=True)
    ...
    try:
        ...
        async def _remove_file_always_fails(*args, **kwargs) -> bool:
            """Internal helper for remove file always fails."""
            return False
