"""Test module for tests test chat closeout."""
...
def test_build_display_blocks_returns_empty_for_empty_answer() -> None:
    """Test build display blocks returns empty for empty answer."""
    assert not build_display_blocks("")


def test_build_display_blocks_extracts_markdown_table() -> None:
    """Test build display blocks extracts markdown table."""
    answer = (
        "Permission Matrix\n\n"
        "| Role | Access |\n"
        "| --- | --- |\n"
...
def test_build_display_blocks_extracts_code_fence() -> None:
    """Test build display blocks extracts code fence."""
...
def test_build_display_blocks_coerces_unfenced_code_only_answer() -> None:
    """Test build display blocks coerces unfenced code only answer."""
    answer = (
...
def test_build_display_blocks_keeps_plain_prose_as_text_block() -> None:
    """Test build display blocks keeps plain prose as text block."""
    answer = (
...
def test_build_display_blocks_extracts_nested_checklist() -> None:
    """Test build display blocks extracts nested checklist."""
    answer = "Checklist:\n\n- [x] Top done\n- [ ] Top todo\n  - Child note\n"
    blocks = build_display_blocks(answer)
    assert len(blocks) == 2
...
def test_build_display_blocks_extracts_quote_block() -> None:
    """Test build display blocks extracts quote block."""
...
def test_build_display_blocks_keeps_indented_list_continuations_out_of_code_blocks() -> None:
    """Test build display blocks keeps indented list continuations out of code blocks."""
    answer = (
...
def test_build_display_blocks_normalizes_disclaimer_with_consistent_callout() -> None:
    """Test build display blocks normalizes disclaimer with consistent callout."""
    answer = (
...
def test_build_display_blocks_normalizes_bold_disclaimer_with_continuation_line() -> None:
    """Test build display blocks normalizes bold disclaimer with continuation line."""
    answer = (
...
def test_build_display_blocks_normalizes_heading_disclaimer_with_body_on_next_line() -> None:
    """Test build display blocks normalizes heading disclaimer with body on next line."""
...
def test_build_display_blocks_normalizes_plain_disclaimer_label_with_next_line_body() -> None:
    """Test build display blocks normalizes plain disclaimer label with next line body."""
    answer = "Disclaimer:\nInformity AI is not a lawyer and this is not legal advice.\n"
    blocks = build_display_blocks(answer)
    assert blocks == [
        {
