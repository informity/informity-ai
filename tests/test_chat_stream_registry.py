"""Test module for tests test chat stream registry."""
...
async def test_mark_stopped_by_request_id_is_idempotent() -> None:
    """Test mark stopped by request id is idempotent."""
...
async def test_mark_stopped_by_stream_id_preserved() -> None:
    """Test mark stopped by stream id preserved."""
    registry = ChatStreamRegistry()
    stop_event = asyncio.Event()
...
async def test_mark_stopped_by_user_requires_matching_chat_id() -> None:
    """Test mark stopped by user requires matching chat id."""
    registry = ChatStreamRegistry()
    stop_event = asyncio.Event()
...
async def test_stale_request_id_does_not_stop_newer_request() -> None:
    """Test stale request id does not stop newer request."""
    registry = ChatStreamRegistry()
...
async def test_mark_stopped_by_user_cancels_active_task() -> None:
    """Test mark stopped by user cancels active task."""
    registry = ChatStreamRegistry()
    gate = asyncio.Event()

    async def _run_forever() -> None:
        """Internal helper for run forever."""
        await gate.wait()
