...
"""Test module for tests test prompt builder."""
...
class TestPromptBuilder:
    ...
    def test_builds_system_message_with_context(self) -> None:
        """Test builds system message with context."""
        chunks = [
            {
    ...
    def test_includes_filename_in_source_label(self) -> None:
        """Test includes filename in source label."""
        chunks = [
            {
                "chunk_text": "Content",
                "filename": "test.pdf",
    ...
    def test_reorders_context_chunks_for_attention_without_renumbering_sources(self) -> None:
        """Test reorders context chunks for attention without renumbering sources."""
        chunks = [
            {
    ...
    def test_includes_page_number_if_present(self) -> None:
        """Test includes page number if present."""
        chunks = [
            {
                "chunk_text": "Content",
                "filename": "test.pdf",
    ...
    def test_includes_section_path_if_present(self) -> None:
        """Test includes section path if present."""
        chunks = [
            {
                "chunk_text": "Content",
                "filename": "test.pdf",
    ...
    def test_includes_year_and_category_in_source_label_when_present(self) -> None:
        """Test includes year and category in source label when present."""
        chunks = [
            {
                "chunk_text": "Content",
    ...
    def test_omits_year_and_category_when_not_present(self) -> None:
        """Test omits year and category when not present."""
        chunks = [
            {
                "chunk_text": "Content",
                "filename": "notes.txt",
    ...
    def test_truncates_long_history(self) -> None:
        ...
        chunks = []
        history = [
            ChatMessage(chat_id="test", role="user", content=f"Question {i}") for i in range(10)
        ]
    ...
    def test_uses_assistant_history_limit_by_mode(self) -> None:
        """Test uses assistant history limit by mode."""
    ...
    def test_uses_researcher_history_limit_by_mode(self) -> None:
        """Test uses researcher history limit by mode."""
        original_default = settings.chat_history_messages
        original_assistant = settings.chat_history_messages_assistant
        original_researcher = settings.chat_history_messages_researcher
        try:
    ...
    def test_uses_fallback_history_limit_when_mode_is_unresolved(self) -> None:
        """Test uses fallback history limit when mode is unresolved."""
        original_default = settings.chat_history_messages
        original_assistant = settings.chat_history_messages_assistant
        original_researcher = settings.chat_history_messages_researcher
        try:
    ...
    def test_token_budget_trim_limits_history_when_profile_is_provided(self) -> None:
        """Test token budget trim limits history when profile is provided."""
        history = [
    ...
    def test_empty_chunks_still_builds_messages(self) -> None:
        """Test empty chunks still builds messages."""
    ...
    def test_system_prompt_includes_context_anchor(self) -> None:
        """Test system prompt includes context anchor."""
        messages = build_messages("Question", [])
        content = messages[0]["content"]
    ...
    def test_builder_does_not_include_research_instructions(self) -> None:
        """Test builder does not include research instructions."""
        messages = build_messages("Question", [])
    ...
    def test_builder_keeps_single_route_prompt_contract(self) -> None:
        """Test builder keeps single route prompt contract."""
        messages = build_messages("Question", [])
        assert "Research mode instructions:" not in messages[0]["content"]

    def test_builder_system_prompt_matches_rag_specialization_composer_exactly(self) -> None:
        """Test builder system prompt matches rag specialization composer exactly."""
    ...
    def test_builder_assistant_mode_system_prompt_matches_composer_exactly(self) -> None:
        """Test builder assistant mode system prompt matches composer exactly."""
    ...
    def test_builder_general_specialization_parity_when_specialization_absent(self) -> None:
        ...
        messages_no_specialization = build_messages(
            "Question", [], chat_mode="researcher", specialization_id=None
        )
    ...
    def test_builder_applies_specialization_overlay_when_specialization_present(self) -> None:
        """Test builder applies specialization overlay when specialization present."""
    ...
    def test_builder_applies_specialization_specific_financial_numeric_discipline(self) -> None:
        """Test builder applies specialization specific financial numeric discipline."""
        messages = build_messages(
            "Question", [], chat_mode="researcher", specialization_id="financial"
        )
    ...
    def test_builder_applies_specialization_specific_technical_non_invention_rule(self) -> None:
        """Test builder applies specialization specific technical non invention rule."""
        messages = build_messages(
            "Question", [], chat_mode="researcher", specialization_id="technical"
        )
    ...
    def test_builder_applies_specialization_specific_legal_compact_contract(self) -> None:
        """Test builder applies specialization specific legal compact contract."""
        messages = build_messages("Question", [], chat_mode="researcher", specialization_id="legal")
    ...
    def test_preserves_assistant_history_verbatim(self) -> None:
        """Test preserves assistant history verbatim."""
        history = [
    ...
    def test_keeps_assistant_history_when_reasoning_only(self) -> None:
        """Test keeps assistant history when reasoning only."""
        history = [
    ...
    def test_includes_output_contract_constraints_when_provided(self) -> None:
        """Test includes output contract constraints when provided."""
        messages = build_messages(
            "Question",
            [],
