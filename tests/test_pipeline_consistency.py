"""Test module for tests test pipeline consistency."""
...
@pytest.mark.asyncio
async def test_chunk_embed_store_fails_on_partial_vector_write(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test chunk embed store fails on partial vector write."""
    ...
    monkeypatch.setattr(
        "informity.indexer.pipeline.chunk_text", lambda *args, **kwargs: [parent_chunk]
    )
    monkeypatch.setattr(
        "informity.indexer.pipeline.create_child_chunks", lambda *args, **kwargs: [child_chunk]
    )
    monkeypatch.setattr(
    ...
    def _embed_texts(texts):
        """Return zero vectors for texts."""
        return [[0.0] * 768 for _ in texts]

    monkeypatch.setattr("informity.indexer.pipeline.embedder.embed_texts", _embed_texts)
    monkeypatch.setattr(
        "informity.indexer.pipeline.vector_store.store_embeddings_async",
        AsyncMock(return_value=0),
    )
    def _delete_by_file_id(file_id):
        """Record deleted vector ids."""
        deleted_vectors.append(file_id)

    monkeypatch.setattr("informity.indexer.pipeline.vector_store.delete_by_file_id", _delete_by_file_id)
