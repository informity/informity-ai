"""Test module for tests test operation state file reindex."""
...
async def _reset_file_reindex_state() -> None:
    """Internal helper for reset file reindex state."""
...
async def test_file_reindex_operation_dedupes_by_file_id() -> None:
    """Test file reindex operation dedupes by file id."""
    first, first_is_new = await op_state.begin_file_reindex_operation(file_id=17, filename="a.txt")
    second, second_is_new = await op_state.begin_file_reindex_operation(
        file_id=17, filename="a.txt"
    )
...
async def test_file_reindex_operation_transitions_to_completed() -> None:
    """Test file reindex operation transitions to completed."""
