"""Test module for tests test routes scan timeout bounds."""
...
def test_resolve_scan_timeout_seconds_caps_plaintext(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    """Test resolve scan timeout seconds caps plaintext."""
    monkeypatch.setattr(
        "informity.api.routes_scan.resolve_timeout_seconds", lambda *args, **kwargs: 600
    )
    sf = ScannedFile(
...
def test_resolve_scan_timeout_seconds_non_plaintext_unchanged(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    """Test resolve scan timeout seconds non plaintext unchanged."""
    monkeypatch.setattr(
        "informity.api.routes_scan.resolve_timeout_seconds", lambda *args, **kwargs: 480
    )
    sf = ScannedFile(
