"""Test module for tests test routes core."""
...
async def test_search_documents_applies_filters_and_limit(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test search documents applies filters and limit."""
    monkeypatch.setattr(routes_search.asyncio, "to_thread", _to_thread)
    monkeypatch.setattr(routes_search.SEARCH_GUARD, "slot", _noop_slot)
    def _mock_embed_query(_query: str) -> list[float]:
        """Return a small embedding vector."""
        return [0.1] * 8

    monkeypatch.setattr(routes_search.embedder, "embed_query", _mock_embed_query)
    monkeypatch.setattr(
        routes_search.vector_store,
        "search_similar",
        _mock_search_similar,
    )
    ...
    response = await routes_search.search_documents(
        SearchRequest(
            query="  find documents  ", limit=2, category="document", file_types=[".pdf"]
        ),
        db=MagicMock(),
    )
...
async def test_search_documents_rejects_empty_query(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test search documents rejects empty query."""
    monkeypatch.setattr(routes_search.SEARCH_GUARD, "slot", _noop_slot)
...
@pytest.mark.asyncio
async def test_get_index_status_aggregates_counts_and_sizes(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test get index status aggregates counts and sizes."""
    ...
    monkeypatch.setattr(
        routes_index, "get_indexed_content_size_bytes", AsyncMock(return_value=4096)
    )
    ...
    monkeypatch.setattr(
        routes_index.vector_store,
        "get_stats",
        lambda: {"total_vectors": 27, "storage_bytes": 12345},
    )
    monkeypatch.setattr(routes_index, "_compute_disk_sizes", lambda: (111, 222))
    monkeypatch.setattr(
        routes_index.op_state,
        "get_reset_state_snapshot",
        AsyncMock(return_value=(False, {"ok": True})),
    )
...
@pytest.mark.asyncio
async def test_get_index_status_short_circuits_while_reset_in_progress(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test get index status short circuits while reset in progress."""
...
async def test_get_diagnostics_returns_system_and_index_stats(
    ...
    """Test get diagnostics returns system and index stats."""
    ...
    monkeypatch.setattr(
        routes_system,
        "llm_engine",
        SimpleNamespace(is_loaded=True, _get_model_path=lambda: model_path),
    )
    ...
    monkeypatch.setattr(
        routes_system, "get_indexed_content_size_bytes", AsyncMock(return_value=2048)
    )
...
async def test_shutdown_rejects_non_localhost() -> None:
    """Test shutdown rejects non localhost."""
...
async def test_get_diagnostics_rejects_non_localhost() -> None:
    """Test get diagnostics rejects non localhost."""
    request = SimpleNamespace(client=SimpleNamespace(host="10.0.0.8"))
    with pytest.raises(HTTPException) as exc_info:
...
async def test_shutdown_allows_localhost() -> None:
    """Test shutdown allows localhost."""
    request = SimpleNamespace(client=SimpleNamespace(host="127.0.0.1"))
...
async def test_get_setup_status_returns_ready_when_required_models_cached(
    ...
    """Test get setup status returns ready when required models cached."""
...
async def test_get_setup_status_reflects_setup_state_file(
    ...
    """Test get setup status reflects setup state file."""
    monkeypatch.setattr(routes_system.settings, "llm_provider", "local_gguf")
...
async def test_get_setup_status_ollama_provider_bypasses_local_setup_gate(
    ...
    """Test get setup status ollama provider bypasses local setup gate."""
    monkeypatch.setattr(routes_system.settings, "app_data_dir", tmp_path)
    ...
    monkeypatch.setattr(
        ...
        lambda: (True, False, "Ollama model not found: qwen3:14b"),
    )
...
async def test_retry_setup_ollama_uses_classic_workflow(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def _fake_setup_workflow(*, tier: str, model_filename: str) -> None:
        """Internal helper for fake setup workflow."""
    ...
    monkeypatch.setattr(
        ...
        lambda _path: (
            {"selected_tier": "small", "model_filename": "Qwen_Qwen3.5-9B-Q4_K_M.gguf"},
            None,
        ),
    )
...
async def test_start_setup_persists_in_progress_state(
    ...
    """Test start setup persists in progress state."""
    monkeypatch.setattr(routes_system.settings, "app_data_dir", tmp_path)
    ...
    def _fake_create_task(coro):  # type: ignore[no-untyped-def]
        """Internal helper for fake create task."""
        coro.close()
        return _DummyTask()
...
async def test_start_setup_rejects_mismatched_model_filename() -> None:
    """Test start setup rejects mismatched model filename."""
    with pytest.raises(HTTPException) as exc_info:
        await routes_system.start_setup(
...
async def test_setup_events_reflect_runtime_state(
    ...
    """Test setup events reflect runtime state."""
    monkeypatch.setattr(routes_system.settings, "app_data_dir", tmp_path)
...
async def test_get_models_catalog_marks_installed_and_default(
    ...
    """Test get models catalog marks installed and default."""
...
async def test_get_models_catalog_marks_quality_installed_for_canonical_35b(
    ...
    """Test get models catalog marks quality installed for canonical 35b."""
    models_dir = tmp_path / "models"
    models_dir.mkdir(parents=True)
...
async def test_get_models_catalog_includes_legacy_quality_model_when_present(
    ...
    """Test get models catalog includes legacy quality model when present."""
    models_dir = tmp_path / "models"
    models_dir.mkdir(parents=True)
    ...
    current_entry = next(
        item for item in catalog.models if item.model_filename == "Qwen3.6-35B-A3B-UD-Q4_K_M.gguf"
    )
...
async def test_download_model_starts_background_operation(
    ...
    """Test download model starts background operation."""
    models_dir = tmp_path / "models"
    models_dir.mkdir(parents=True)
    monkeypatch.setattr(routes_system.settings, "models_dir", models_dir)
    ...
    response = await routes_system.download_model(
        ModelActionRequest(model_filename="Qwen3-14B-Q5_K_M.gguf")
    )
    assert response.accepted is True
...
async def test_set_default_model_requires_installed_file_and_updates_config(
    ...
    """Test set default model requires installed file and updates config."""
    app_data_dir = tmp_path / "app-data"
    models_dir = tmp_path / "models"
    app_data_dir.mkdir(parents=True)
    models_dir.mkdir(parents=True)
    ...
    response = await routes_system.set_default_model(
        ModelActionRequest(model_filename="Qwen3-14B-Q5_K_M.gguf")
    )
    assert response.accepted is True
...
async def test_model_cancel_reflect_runtime_state(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test model cancel reflect runtime state."""
...
async def test_remove_model_rejects_active_model(
    ...
    """Test remove model rejects active model."""
    models_dir = tmp_path / "models"
    models_dir.mkdir(parents=True)
    (models_dir / "Qwen3-14B-Q5_K_M.gguf").write_bytes(b"x")
    monkeypatch.setattr(routes_system.settings, "models_dir", models_dir)
...
async def test_remove_model_allows_legacy_alias_when_newer_alias_is_active(
    ...
    """Test remove model allows legacy alias when newer alias is active."""
    models_dir = tmp_path / "models"
    models_dir.mkdir(parents=True)
    ...
    monkeypatch.setattr(routes_system.settings, "models_dir", models_dir)
    monkeypatch.setattr(
        routes_system.settings, "llm_model_filename", "Qwen3.6-35B-A3B-UD-Q4_K_M.gguf"
    )
    monkeypatch.setattr(routes_system, "_model_task", None)
    monkeypatch.setattr(routes_system, "_setup_task", None)

    response = await routes_system.remove_model(
        ModelActionRequest(model_filename="Qwen3.5-35B-A3B-Q4_K_M.gguf")
    )
    assert response.accepted is True
...
async def test_remove_model_rejects_while_setup_task_in_progress(
    ...
    """Test remove model rejects while setup task in progress."""
    models_dir = tmp_path / "models"
    models_dir.mkdir(parents=True)
    (models_dir / "Qwen_Qwen3.5-9B-Q4_K_M.gguf").write_bytes(b"x")
    monkeypatch.setattr(routes_system.settings, "models_dir", models_dir)
    ...
    response = await routes_system.remove_model(
        ModelActionRequest(model_filename="Qwen_Qwen3.5-9B-Q4_K_M.gguf")
    )
...
async def test_remove_model_deletes_installed_file(
    ...
    """Test remove model deletes installed file."""
    models_dir = tmp_path / "models"
    models_dir.mkdir(parents=True)
...
async def test_get_diagnostics_summary_aggregates_rows(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test get diagnostics summary aggregates rows."""
    rows = [
        {
            "type": "user",
            "query_type": "focused",
    ...
    monkeypatch.setattr(routes_system, "get_connection", AsyncMock(return_value=fake_db))
    monkeypatch.setattr(
        routes_system, "get_diagnostics_metrics_since", AsyncMock(return_value=rows)
    )

    summary = await routes_system.get_diagnostics_summary(
        days=7, type_filter=None, run_id_filter=None
    )
...
@pytest.mark.asyncio
async def test_get_diagnostics_summary_handles_empty_dataset(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test get diagnostics summary handles empty dataset."""
    fake_db = MagicMock()
    fake_db.close = AsyncMock()
    monkeypatch.setattr(routes_system, "get_connection", AsyncMock(return_value=fake_db))
    ...
    summary = await routes_system.get_diagnostics_summary(
        days=30, type_filter="user", run_id_filter="run-x"
    )
...
@pytest.mark.asyncio
async def test_get_diagnostics_summary_normalizes_non_canonical_fields(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test get diagnostics summary normalizes non canonical fields."""
    rows = [
        {
            "type": "UNKNOWN_TYPE",
