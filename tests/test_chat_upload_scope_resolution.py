"""Test module for tests test chat upload scope resolution."""
...
def _attachment(
    ...
    *,
    ...
    """Internal helper for attachment."""
...
def test_extract_filename_candidates_reads_dot_tokens_and_quotes() -> None:
    """Test extract filename candidates reads dot tokens and quotes."""
...
def test_looks_per_file_separate_request_detects_each_document_prompt() -> None:
    """Test looks per file separate request detects each document prompt."""
    assert (
        routes_chat._looks_per_file_separate_request("Summarize each document separately") is True
    )


def test_looks_per_file_separate_request_noop_for_generic_prompt() -> None:
    """Test looks per file separate request noop for generic prompt."""
...
def test_build_per_file_separate_guidance_requires_multi_file_scope() -> None:
    """Test build per file separate guidance requires multi file scope."""
...
def test_build_per_file_separate_guidance_builds_exact_count_contract() -> None:
    """Test build per file separate guidance builds exact count contract."""
...
def test_resolve_upload_scope_exact_case_insensitive_match() -> None:
    """Test resolve upload scope exact case insensitive match."""
    attachments = [
...
def test_resolve_upload_scope_unique_partial_match() -> None:
    """Test resolve upload scope unique partial match."""
    attachments = [
...
def test_resolve_upload_scope_ambiguous_duplicate_filename_requires_clarification() -> None:
    """Test resolve upload scope ambiguous duplicate filename requires clarification."""
    attachments = [
...
def test_build_retrieval_scope_uses_earliest_active_upload_anchor() -> None:
    """Test build retrieval scope uses earliest active upload anchor."""
...
def test_build_retrieval_scope_keeps_session_anchor_when_original_anchor_deleted() -> None:
    """Test build retrieval scope keeps session anchor when original anchor deleted."""
    scope_kind, scope_key = routes_chat._build_retrieval_scope(
        chat_mode="researcher",
...
def test_build_retrieval_scope_resets_anchor_after_full_depletion() -> None:
    """Test build retrieval scope resets anchor after full depletion."""
    scope_kind, scope_key = routes_chat._build_retrieval_scope(
        chat_mode="researcher",
        scoped_file_ids=None,
        upload_attachments=[
            _attachment(
                "up-4", "new.pdf", uploaded_at=datetime(2026, 4, 19, 4, 20, 20, tzinfo=UTC)
            ),
        ],
        upload_attachments_all=[
            _attachment(
...
def test_build_retrieval_scope_adds_subset_suffix_for_selected_uploads() -> None:
    """Test build retrieval scope adds subset suffix for selected uploads."""
    scope_kind, scope_key = routes_chat._build_retrieval_scope(
        chat_mode="researcher",
        scoped_file_ids=None,
        upload_attachments=[
...
def test_filter_history_for_scope_isolates_upload_from_indexed_context() -> None:
    """Test filter history for scope isolates upload from indexed context."""
    history = [
        ChatMessage(
            chat_id="chat-1",
            role="user",
...
def test_filter_history_for_scope_respects_indexed_generation_boundaries() -> None:
    """Test filter history for scope respects indexed generation boundaries."""
    history = [
        ChatMessage(
            chat_id="chat-1",
            role="user",
...
def test_resolve_retrieval_context_scope_key_resets_on_topic_shift() -> None:
    """Test resolve retrieval context scope key resets on topic shift."""
    history = [
        ChatMessage(
            chat_id="chat-1",
            role="assistant",
...
def test_resolve_retrieval_context_scope_key_does_not_reset_without_explicit_shift_cue() -> None:
    """Test resolve retrieval context scope key does not reset without explicit shift cue."""
    history = [
        ChatMessage(
            chat_id="chat-1",
            role="assistant",
...
def test_resolve_retrieval_context_scope_key_does_not_reset_on_back_to_cue() -> None:
    """Test resolve retrieval context scope key does not reset on back to cue."""
    history = [
        ChatMessage(
            chat_id="chat-1",
            role="assistant",
...
def test_resolve_retrieval_context_scope_key_resets_on_upload_to_indexed_transition() -> None:
    """Test resolve retrieval context scope key resets on upload to indexed transition."""
    history = [
        ChatMessage(
            chat_id="chat-1",
            role="assistant",
