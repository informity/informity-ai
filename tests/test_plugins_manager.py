"""Test module for tests test plugins manager."""
...
def _write_manifest(plugin_dir: Path, payload: dict[str, object]) -> None:
    """Internal helper for write manifest."""
...
def test_plugin_manifest_requires_hook_permissions() -> None:
    """Test plugin manifest requires hook permissions."""
...
def test_discover_plugins_reports_enabled_and_invalid_plugins(tmp_path: Path) -> None:
    """Test discover plugins reports enabled and invalid plugins."""
...
def test_install_toggle_and_global_kill_switch(tmp_path: Path) -> None:
    """Test install toggle and global kill switch."""
    app_data_dir = tmp_path / "app-data"
    ...
    legal_after_kill_switch = next(
        item for item in disabled_inventory.plugins if item.id == "legal"
    )
