"""Test module for tests test pipeline file limits."""
...
@pytest.mark.asyncio
async def test_index_file_scanned_file_too_large_skips_extraction(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test index file scanned file too large skips extraction."""
...
async def test_index_file_pathological_text_line_skips_extraction(
    ...
    """Test index file pathological text line skips extraction."""
    text_path = tmp_path / "pathological.txt"
    text_path.write_text(("A" * 250_000) + "\nsmall\n", encoding="utf-8")
    monkeypatch.setattr(
        "informity.indexer.pipeline.get_max_file_size_bytes", lambda: 1024 * 1024 * 100
    )
