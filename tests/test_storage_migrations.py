...
async def test_migrate_legacy_upload_storage_layout_moves_chat_and_translate_uploads(
    ...
    legacy_chat_file = app_data_dir / 'storage' / 'upload' / 'chat' / 'chat-1' / 'upload-1' / 'chat.txt'
    legacy_translate_file = app_data_dir / 'storage' / 'upload' / 'translate' / 'tx-1' / 'translate.txt'
    ...
    try:
        ...
        assert [str(row['source_item_id']) for row in updated_rows] == [str(item['path']) for item in updated_rows]
