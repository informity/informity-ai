"""Test module for tests test chat upload policy routes."""
...
async def _insert_indexed_file(db, *, path: str, filename: str, content_hash: str) -> int:
    """Internal helper for insert indexed file."""
...
async def test_chat_rejects_uploads_in_assistant_mode(
    ...
    """Test chat rejects uploads in assistant mode."""
...
async def test_upload_endpoint_rejects_uploads_for_assistant_locked_chat(
    ...
    """Test upload endpoint rejects uploads for assistant locked chat."""
    monkeypatch.setattr(settings, "app_data_dir", tmp_path / "app-data")
...
async def test_chat_rejects_mixed_library_scope_and_uploads(
    ...
    """Test chat rejects mixed library scope and uploads."""
    monkeypatch.setattr(settings, "app_data_dir", tmp_path / "app-data")
    ...
    try:
        ...
        assert (
            ...
            == "Mixed library scope and chat uploads are not supported in one turn."
        )
    finally:
        await db.close()
...
async def test_chat_rejects_scoped_file_ids_in_assistant_mode(
    ...
    """Test chat rejects scoped file ids in assistant mode."""
    monkeypatch.setattr(settings, "app_data_dir", tmp_path / "app-data")
    ...
    try:
        ...
        assert exc_info.value.status_code == 409
        assert (
            exc_info.value.detail == "Scoped file retrieval is available only in Researcher mode."
        )
    finally:
        await db.close()
...
async def test_chat_explicit_scoped_upload_ids_require_existing_uploads(
    ...
    """Test chat explicit scoped upload ids require existing uploads."""
    monkeypatch.setattr(settings, "app_data_dir", tmp_path / "app-data")
...
async def test_chat_explicit_scoped_upload_ids_reject_indexing_upload(
    ...
    """Test chat explicit scoped upload ids reject indexing upload."""
    monkeypatch.setattr(settings, "app_data_dir", tmp_path / "app-data")
    ...
    try:
        ...
        assert (
            exc_info.value.detail
            == "Selected uploaded file is still indexing. Please retry in a moment."
        )
    finally:
        await db.close()
...
async def test_chat_explicit_scoped_upload_ids_reject_missing_upload_ids(
    ...
    """Test chat explicit scoped upload ids reject missing upload ids."""
    monkeypatch.setattr(settings, "app_data_dir", tmp_path / "app-data")
...
async def test_chat_allows_follow_up_when_role_is_omitted_but_chat_role_is_locked(
    ...
    """Test chat allows follow up when role is omitted but chat role is locked."""
    monkeypatch.setattr(settings, "app_data_dir", tmp_path / "app-data")
    ...
    try:
        ...
        def _raise_after_role_lock(**_kwargs):
            """Internal helper for raise after role lock."""
            raise RuntimeError("after_role_lock")

        monkeypatch.setattr(
            routes_chat, "resolve_retrieval_context_scope_key", _raise_after_role_lock
        )
...
async def test_chat_allows_follow_up_when_user_role_missing_but_assistant_role_locked(
    ...
    """Test chat allows follow up when user role missing but assistant role locked."""
    monkeypatch.setattr(settings, "app_data_dir", tmp_path / "app-data")
...
async def test_chat_coerces_follow_up_mode_to_locked_chat_mode(
    ...
    """Test chat coerces follow up mode to locked chat mode."""
    monkeypatch.setattr(settings, "app_data_dir", tmp_path / "app-data")
    ...
    try:
        ...
        persisted_user = [
            msg for msg in history if msg.role == ChatRole.USER and not bool(msg.is_internal)
        ]
...
async def test_get_chat_preserves_specialization_id_from_db_rows(
    ...
    """Test get chat preserves specialization id from db rows."""
    monkeypatch.setattr(settings, "app_data_dir", tmp_path / "app-data")
...
async def test_chat_coerces_follow_up_role_to_locked_chat_role(
    ...
    """Test chat coerces follow up role to locked chat role."""
    monkeypatch.setattr(settings, "app_data_dir", tmp_path / "app-data")
