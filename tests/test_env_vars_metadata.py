"""Test module for tests test env vars metadata."""
...
def test_env_vars_metadata_covers_all_settings_fields() -> None:
    """Test env vars metadata covers all settings fields."""
    response = get_env_vars_response(Settings())
    env_names = {variable.name for group in response.groups for variable in group.variables}

    expected_setting_env_names = {f"INFORMITY_{field.upper()}" for field in Settings.model_fields}
...
def test_env_vars_metadata_includes_runtime_environment_group() -> None:
    """Test env vars metadata includes runtime environment group."""
    response = get_env_vars_response(Settings())
    runtime_group = next(
        (group for group in response.groups if group.title == "Runtime Environment"), None
    )
...
def test_env_vars_metadata_redacts_runtime_secrets(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    """Test env vars metadata redacts runtime secrets."""
    ...
    assert runtime_group is not None
    token_item = next(
        (item for item in runtime_group.variables if item.name == "INFORMITY_TAURI_SESSION_TOKEN"),
        None,
    )
    assert token_item is not None
    assert token_item.current_value == "(set, redacted)"
