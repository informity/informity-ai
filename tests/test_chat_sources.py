"""Test module for tests test chat sources."""
...
def test_merge_sources_deduplicates_by_path_and_filename() -> None:
    """Test merge sources deduplicates by path and filename."""
    source_map: dict[tuple[str, str], ChatSourceReference] = {}
    merge_sources(
        source_map,
        [
            ChatSourceReference(
                filename="a.txt", path="/a.txt", chunk_preview="a", relevance_score=0.5
            ),
            ChatSourceReference(
                filename="a.txt", path="/a.txt", chunk_preview="updated", relevance_score=0.9
            ),
        ],
    )
...
def test_serialize_sources_returns_json_payloads() -> None:
    """Test serialize sources returns json payloads."""
