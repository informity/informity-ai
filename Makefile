...
.DEFAULT_GOAL := help
.PHONY: help run dev test lint format baseline diagnostics-evaluate diagnostics-analyze diagnostics-pipeline diagnostics-stop reset-db reset-vectors reset-all clean-data clean install uninstall frontend frontend-build app qa-quick qa-full qa-security qa-secrets qa-lint qa-typecheck qa-docs smoke-basic smoke-infra smoke-pdf maintenance-index-check maintenance-index-repair maintenance-download-nltk maintenance-reinstall-packages maintenance-chunk-structure maintenance-legacy-chunks maintenance-orphaned-chunks maintenance-migrate-hf-cache
...
# Use local data directory for development (absolute path so settings persist regardless of cwd)
...
install: ## Install dependencies and download models (embedding, reranker, optional LLM) into app data
	./scripts/install.sh

uninstall: ## Remove all user data, downloaded models, and .venv (fresh distribution state)
	./scripts/uninstall.sh

full-reset-install: ## Full reset (data, venv, node_modules, .cache except chat/classifier model dirs), install, and test
...
dev: ## Run with auto-reload for development (code changes restart the server)
	INFORMITY_DEV_RELOAD=true uv run python -m informity.main
...
clean-data: ## Remove unnecessary files from data/ (locks, .no_exist under HF cache). Keeps config, db, vectors, models, embedding and reranker.
	...
	rm -rf "$(APP_DATA_DIR)/cache/huggingface/hub/.locks"
	find "$(APP_DATA_DIR)/cache/huggingface/hub" -type d -name ".no_exist" -exec rm -rf {} + 2>/dev/null || true
